...
CSV_PATHS = {
    ...
    'products': 'C:/Project/think41/data/products.csv',
...
def clean_users_csv():
    ...
    # Drop duplicates based on email, keep the first occurrence
...
def clean_orders_csv(users_csv_path, orders_csv_path, output_orders_clean_path):
    ...
    # Filter orders where user_id is in valid_user_ids
...
def clean_order_items_csv(users_csv_path, orders_csv_path, order_items_path, cleaned_order_items_path, expected_columns=11):
    ...
    # Filter rows where user_id and order_id are valid
    df_filtered = df[(df['user_id'].isin(valid_user_ids)) & (df['order_id'].isin(valid_order_ids))]

    # Trim columns if more than expected
...
def main():
    try:
        ...
        # Step 4: Connect to DB and load all data
        conn = psycopg2.connect(**DB_CONFIG)
        conn.autocommit = False
        cursor = conn.cursor()

        for table, csv_file in TABLE_CSV_MAP.items():
            load_csv_to_table(cursor, table, csv_file)
